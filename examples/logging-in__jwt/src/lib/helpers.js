...
function privado (res, token) {
  console.log('private content request %s %s', req.method, req.url)
  res.writeHead(200, {
    'content-type': 'text/html',
    authorization: token
  })
...
function validate (req, res, callback) {
  console.log('validating request to %s %s', req.method, req.url)
...
function logout (req, res, callback) {
  ...
  var decoded = verify(token)
  if (decoded) {
    // otherwise someone can force the server to crash by sending a bad token!
